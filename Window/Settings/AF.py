...
Version:
    0.1.1
Description:
    Contains classes representing various settings related to AF (Anisotropic
    Filtering), and LOD Bias, along with a base class for all graphics 
    settings.
...
class Anistropic_Filtering_Enable(BaseSetting):
    # Class representing the Anistropic_Filtering setting.

    def __init__(self):
        """
        Initializes the Enable Anistropic_Filtering setting.

        Returns:

            str: Title of the setting (Anistropic_Filtering_Enable) encoded in HTML
            tuple: Set of immutable options for the current setting (Anistropic_Filtering_Enable)
            str: Tooltip header of the setting (Anistropic_Filtering_Enable) encoded in HTML
            str: Tooltip body of the setting (Anistropic_Filtering_Enable) encoded in HTML

        Examples:

        Instantiate the Enable Anisotropic Filtering setting:
            >>> af__enable_setting = Anistropic_Filtering_Enable()
        ...
        """
        title = f'<p>Enable Anistropic Filtering&nbsp;&nbsp;&nbsp;&nbsp;{u"\u2753"}</p>'
        options = ("Select Option...", "Enable", "Enable (D3D9)", "Disable")
        tooltip_header = f'<h3 {header}>Select the level of Anistropic Filtering:</h3>'
        tooltip_body = f'<p {body_top}>Anisotropic filtering is a texture filtering technique that enhances the clarity of distant textures in 3D rendering by improving the sharpness of textures viewed at oblique angles</p><p {body_bot}>Enable - Turn on Anistropic Filtering<br>Disable - Turn off Anistropic Filtering</p>'
        rule = False
        super().__init__(title, options, tooltip_header, tooltip_body, rule)


class Anistropic_Filtering_Level(BaseSetting):
    # Class representing the Anistropic_Filtering_Level setting.

    def __init__(self):
        """
        Initializes the Anistropic_Filtering_Level setting.

        Returns:

            str: Title of the setting (Anistropic_Filtering_Level) encoded in HTML
            tuple: Set of immutable options for the current setting (Anistropic_Filtering_Level)
            str: Tooltip header of the setting (Anistropic_Filtering_Level) encoded in HTML
            str: Tooltip body of the setting (Anistropic_Filtering_Level) encoded in HTML

        Examples:

        Instantiate the Anistropic_Filtering_Level setting:
            >>> af_level_setting = Anistropic_Filtering_Level()
        ...
        """
        title = f'<p>&nbsp;&nbsp;&nbsp;&nbsp;-Anistropic Filtering Level&nbsp;&nbsp;&nbsp;&nbsp;{u"\u2753"}</p>'
...
class Anistropic_Filtering_Level_D3D9(BaseSetting):
    # Class representing the Anistropic_Filtering_Level_D3D9 setting.

    def __init__(self):
        """
        Initializes the Anistropic_Filtering_Level_D3D9 setting.

        Returns:

            str: Title of the setting (Anistropic_Filtering_Level_D3D9) encoded in HTML
            tuple: Set of immutable options for the current setting (Anistropic_Filtering_Level_D3D9)
            str: Tooltip header of the setting (Anistropic_Filtering_Level_D3D9) encoded in HTML
            str: Tooltip body of the setting (Anistropic_Filtering_Level_D3D9) encoded in HTML

        Examples:

        Instantiate the Anistropic_Filtering_Level_D3D9 setting:
            >>> af_level_d3d9_setting = Anistropic_Filtering_Level_D3D9()
        ...
        """
        title = f'<p>&nbsp;&nbsp;&nbsp;&nbsp;-Anistropic Filtering Level (D3D9 Applications)&nbsp;&nbsp;&nbsp;&nbsp;{u"\u2753"}</p>'
        options = ("Select Option...", "x16", "x8", "x4", "x2", "x1")
